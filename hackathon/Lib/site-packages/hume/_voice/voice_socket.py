...
class VoiceSocket:
    ...
    async def send_file(
        ...
        """Send a file on the `VoiceSocket`.
        ...
        Args:
            ...
            configs (Optional[List[ModelConfigBase]]): List of model configurations.
                ...
                set when initializing the `VoiceSocket`.

        Returns:
            Any: Response from the voice API.
        """
    ...
    async def send_bytes(
        ...
        """Send raw bytes on the `VoiceSocket`.
    ...
    async def send_facemesh(
        ...
        """Send facemesh landmarks on the `VoiceSocket`.
    ...
    async def reset_stream(self) -> Any:
        ...
        A sliding window of context is maintained for the lifetime of your WebSocket connection.
        Call this method when some media has been fully processed and you want to continue using the same
        WebSocket connection without leaking context across media samples.
    ...
    async def get_job_details(self) -> Any:
        """Get details associated with the current WebSocket connection.
